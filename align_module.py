...
class AlignProperties(PropertyGroup):
    align_point_obj1: FloatVectorProperty(
        name="Align Point Obj1", 
        subtype='XYZ', 
        default=(0.0, 0.0, 0.0),
        precision=4
    )
    align_point_obj2: FloatVectorProperty(
        name="Align Point Obj2", 
        subtype='XYZ', 
        default=(0.0, 0.0, 0.0),
        precision=4
    )
    align_direction: EnumProperty(
        ...
        items=[
            ('OBJ2_TO_OBJ1', "Object 2 to Object 1", "Move Object 2 to align with Object 1"),
            ('OBJ1_TO_OBJ2', "Object 1 to Object 2", "Move Object 1 to align with Object 2")
        ],
...
class SetAlignPointOperator(Operator):
    ...
    def execute(self, context):
        ...
        if len(selected_verts) == 1:
            ...
            local_coord = (selected_edges[0].verts[0].co + selected_edges[0].verts[1].co) / 2
...
class AlignObjectsOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        self.report({'INFO'}, f"New location of {obj_to_move.name}: {obj_to_move.matrix_world.translation}")
